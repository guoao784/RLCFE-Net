...
def get_nms_input(graph,class_num=1,output_name="Concat_985"):
	# input
...
if __name__ == "__main__":
	onnx_path = "./runs/Efficient.NMS.plugin/oursixbest.onnx"
	graph = gs.import_onnx(onnx.load(onnx_path))

	# 添加op得到Efficient NMS plugin的input
	graph = get_nms_input(graph,class_num=6,output_name="Concat_985")

	# 添加Efficient NMS plugin
	graph = create_and_add_plugin_node(graph, 100)

	# 保存图结构
	onnx.save(gs.export_onnx(graph),"./runs/Efficient.NMS.plugin/oursixbest_nms.onnx")
